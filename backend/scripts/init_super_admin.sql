-- 初始化超级管理员账户的SQL脚本
-- 首先确保超级管理员角色已存在
-- 如果users表还没有super_admin角色，需要先更新表结构
...
VALUES ('super_admin', '$2b$10$N9qo8uLOickgx2ZMRZoMy.MrqK3a7LdJ3s5A6J3kK3s5A6J3kK3s5A', 'super_admin', TRUE, 'idle');

-- 更新超级管理员密码为: admin123
-- 注意：在生产环境中请使用更安全的密码
UPDATE users 
SET password = '$2b$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi' 
WHERE username = 'super_admin' AND role = 'super_admin';
...
-- 显示创建结果
SELECT '超级管理员功能初始化完成' as message;
SELECT '默认账户: super_admin / admin123' as account_info;